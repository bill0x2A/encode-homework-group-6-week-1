...
import { deployNewContract, setupScripts } from "../helpers";

async function main() {
  const contractAddress = await deployNewContract();
  const { ballotContract } = await setupScripts(contractAddress);
